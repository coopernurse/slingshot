...
class TestCiCheckFailing:
    def test_failing_check_posts_comment_and_transitions_to_implement(self):
        ...
        with (
            patch.object(daemon, "_transition_label") as mock_tx,
            patch("slingshot.daemon.gh.issue_list", return_value=[issue]),
            patch(
                "slingshot.daemon.gh.pr_list_by_head",
                return_value=[
                    {"number": 10, "state": "OPEN", "mergedAt": None},
                ],
            ),
            patch(
                "slingshot.daemon.gh.pr_check_status",
                return_value={
                    "sha": "abc1234567890",
                    "checks": [
                        {
...
class TestHandleAgentFailureBlocking:
    def test_repeated_failures_with_interleaved_claims_eventually_block(self):
        ...
        daemon = Daemon(cfg)
        ws = WorkerState(
            repo, {"number": 8, "title": "t", "body": "s"}, "slingshot:implement"
        )
...
class TestDoImplementReworkRouting:
    ...
    def test_stale_fail_comments_skip_agent_and_advance(self, tmp_path):
        ...
        with (
            ...
            patch("slingshot.daemon.gh.pr_list_by_head", return_value=[{"number": 10}]),
            patch("slingshot.daemon.gh.pr_comments", return_value=[stale]),
            patch(
                "slingshot.daemon.git.branch_last_commit_epoch",
    ...
    def test_fresh_fail_comment_runs_rework_and_skips_existing_pr_create(
        ...
        with (
            ...
            patch(
                "slingshot.daemon.git.create_worktree_from_remote",
                return_value=mocks["worktree"],
            ),
            patch("slingshot.daemon.git.has_changes", side_effect=mocks["has_changes"]),
    ...
    def test_fresh_scenario_creates_pr_when_none_exists(self, tmp_path):
        ...
        with (
            patch.object(daemon, "_transition_label") as mock_tx,
            patch("slingshot.daemon.git.remote_branch_exists", return_value=False),
            patch("slingshot.daemon.gh.pr_list_by_head", return_value=[]),
            patch(
                "slingshot.daemon.git.create_worktree", return_value=mocks["worktree"]
            ),
            patch("slingshot.daemon.git.has_changes", side_effect=mocks["has_changes"]),
            patch("slingshot.daemon._run_agent", return_value=(0, "")),
            patch("slingshot.daemon.git.commit_changes"),
            patch("slingshot.daemon.git.push_branch"),
            patch("slingshot.daemon.gh.repo_default_branch", return_value="main"),
            patch("slingshot.daemon.gh.pr_create") as m_pr_create,
        ):
            daemon._do_implement(ws)
...
class TestDoReviewMultiSuccess:
    def test_all_models_succeed_synthesis_runs_and_transitions(self, tmp_path):
        ...
        syn_output = (
            "```json\n"
            '{"verdict": "pass", "voters": {"pass": 3, "fail": 0},'
        ...
        with (
            ...
            patch("slingshot.daemon.git.worktree_path", return_value=worktree),
            ...
            patch("slingshot.daemon.gh.pr_mergeable", return_value="MERGEABLE"),
            patch("slingshot.daemon.review_items.fetch_items", return_value=([], [])),
            patch("slingshot.daemon.review_items.qualifying", return_value=[]),
            patch("slingshot.daemon.review_items.partition", return_value=([], [], [])),
            patch("slingshot.daemon.ThreadPoolExecutor", mock_exec_class),
            patch("slingshot.daemon.as_completed", mock_as_completed),
            patch("slingshot.daemon._run_agent", return_value=(0, syn_output)),
    ...
    def test_synthesis_fail_to_implement_label(self, tmp_path):
        ...
        syn_output = (
            "```json\n"
            '{"verdict": "fail", "voters": {"pass": 1, "fail": 1},'
            ' "sections": {"spec_fidelity": {"status": "fail",'
            ' "notes": "broken"}}, "summary": "needs work"}\n'
            "```"
        )
...
class TestDoReviewMultiFailure:
    def test_nonzero_exit_kills_siblings_and_reports_exit_code(self, tmp_path):
        ...
        with (
            patch.object(daemon, "_handle_agent_failure") as mock_fail,
    ...
    def test_synthesis_exit_nonzero_reports_synthesis_exit(self, tmp_path):
        ...
        with (
            ...
            patch("slingshot.daemon.ThreadPoolExecutor", mock_exec_class),
            patch("slingshot.daemon.as_completed", mock_as_completed),
            patch("slingshot.daemon._run_agent", return_value=(1, "synthesis failed")),
            patch("slingshot.daemon._write_agent_log"),
            patch("slingshot.daemon._copy_file"),
        ):
            daemon._do_review_multi(
    ...
    def test_synthesis_no_verdict_reports_no_synthesis_verdict(self, tmp_path):
        ...
        with (
            ...
            patch("slingshot.daemon.ThreadPoolExecutor", mock_exec_class),
            patch("slingshot.daemon.as_completed", mock_as_completed),
            patch("slingshot.daemon._run_agent", return_value=(0, "no json here")),
            patch("slingshot.daemon._write_agent_log"),
            patch("slingshot.daemon._copy_file"),
        ):
            daemon._do_review_multi(
...
class TestItemsBounce:
    def test_review_to_implement_on_unaddressed_items(self):
        ...
        with (
            patch.object(daemon, "_transition_label") as mock_tx,
            patch(
                "slingshot.daemon.review_items.fetch_items", return_value=([item], [])
            ),
            patch("slingshot.daemon.review_items.qualifying", return_value=[item]),
            patch(
                "slingshot.daemon.review_items.partition", return_value=([item], [], [])
            ),
            patch(
                "slingshot.daemon.review_items.get_newest_item_epoch",
    ...
    def test_no_bounce_when_no_unaddressed(self):
        ...
        with (
            ...
            patch(
                "slingshot.daemon.review_items.get_newest_item_epoch", return_value=0
            ),
        ):
            daemon._check_items_bounce(repo, 1, 10, "slingshot:review")
...
class TestAwaitingChecks:
    ...
    def test_unknown_mergeable_no_transition_below_threshold(self):
        ...
        with (
            ...
            patch("slingshot.daemon.gh.pr_mergeable", return_value="UNKNOWN"),
            patch("slingshot.daemon.review_items.fetch_items", return_value=([], [])),
            patch("slingshot.daemon.review_items.qualifying", return_value=[]),
            patch("slingshot.daemon.review_items.partition", return_value=([], [], [])),
        ):
    ...
    def test_none_mergeable_bails_after_threshold(self):
        ...
        with (
            ...
            patch("slingshot.daemon.gh.pr_mergeable", return_value=None),
            patch("slingshot.daemon.review_items.fetch_items", return_value=([], [])),
            patch("slingshot.daemon.review_items.qualifying", return_value=[]),
            patch("slingshot.daemon.review_items.partition", return_value=([], [], [])),
        ):
    ...
    def test_counter_cleared_when_mergeable_becomes_known(self):
        ...
        with (
            ...
            patch("slingshot.daemon.gh.pr_mergeable", return_value="CONFLICTING"),
            patch("slingshot.daemon.review_items.fetch_items", return_value=([], [])),
            patch("slingshot.daemon.review_items.qualifying", return_value=[]),
            patch("slingshot.daemon.review_items.partition", return_value=([], [], [])),
        ):
...
class TestBlockedUnblock:
    def test_unblocks_when_new_items_exist(self):
        ...
        item = create_test_item(
            alias="S1", kind="inline", created_at="2025-01-03T00:00:00Z"
        )
        old_issue_comments = [
            {
        ...
        with (
            ...
            patch("slingshot.daemon.review_items.qualifying", return_value=[item]),
            patch(
                "slingshot.daemon.gh.issue_comments", return_value=old_issue_comments
            ),
            patch("slingshot.daemon.gh.issue_comment_create"),
        ):
...
class TestDisputedReplies:
    def test_disputes_inline_item(self):
        ...
        item = create_test_item(
            alias="S1", kind="inline", comment_id="1001", thread_node_id="node1"
        )
    ...
    def test_disputes_conversation_item(self):
        ...
        item = create_test_item(
            alias="S2", kind="conversation", conversation_comment_id="2002"
        )
    ...
    def test_disputes_mixed_items(self):
        ...
        inline_item = create_test_item(
            alias="S1", kind="inline", comment_id="1001", thread_node_id="n1"
        )
        conv_item = create_test_item(
            alias="S2", kind="conversation", conversation_comment_id="2002"
        )
        unsolved = [
        ...
        with (
            patch("slingshot.daemon.gh.pr_review_reply") as mock_reply,
            patch("slingshot.daemon.gh.pr_comment_create") as mock_comment,
        ):
...
class TestAbortLabelMismatch:
    def test_label_mismatch_aborts_worker(self):
        ...
        ws = WorkerState(
            repo, {"number": 1, "title": "t", "body": "s"}, "slingshot:implement"
        )
        ws.proc = None

        with (
            patch(
                "slingshot.daemon.gh.issue_get",
                return_value={
                    "number": 1,
                    "state": "OPEN",
    ...
    def test_no_abort_when_labels_match(self):
        ...
        ws = WorkerState(
            repo, {"number": 1, "title": "t", "body": "s"}, "slingshot:review"
        )
        ws.proc = None
